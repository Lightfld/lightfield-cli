...
var accountCreate = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		&requestflag.Flag[map[string]any]{
			Name:     "fields",
			Usage:    "Field values for the new account. System fields use a `$` prefix (e.g. `$name`, `$website`); custom attributes use their bare slug (e.g. `tier`, `renewalDate`). Required: `$name` (string). Fields of type `single_select` or `multi_select` require a valid option ID from the field's `typeConfiguration.options` — call the [definitions endpoint](/api/resources/account/methods/definitions) to discover available fields and option IDs. See [Fields and relationships](/using-the-api/fields-and-relationships/) for value type details.",
			Required: true,
			BodyPath: "fields",
		},
		&requestflag.Flag[map[string]any]{
	...
	"fields": {
		...
		&requestflag.InnerFlag[any]{
			...
			Usage:      "Employee count range (SINGLE_SELECT). Pass the option ID from the field definition.",
			InnerField: "$headcount",
		},
		&requestflag.InnerFlag[any]{
			Name:       "fields.industry",
			Usage:      "Industries the account operates in (MULTI_SELECT). Pass option IDs from the field definition.",
			InnerField: "$industry",
		},
		&requestflag.InnerFlag[any]{
		...
		&requestflag.InnerFlag[any]{
			...
			Usage:      "Most recent funding round type (SINGLE_SELECT). Pass the option ID from the field definition.",
		...
		&requestflag.InnerFlag[any]{
			...
			Usage:      "Primary address (ADDRESS type). Object with optional keys: `street`, `street2`, `city`, `state`, `postalCode`, `country` (2-letter ISO code).",
...
var accountUpdate = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[map[string]any]{
			Name:     "fields",
			Usage:    "Field values to update — only provided fields are modified; omitted fields are left unchanged. System fields use a `$` prefix (e.g. `$name`); custom attributes use their bare slug. Select-type fields require a valid option ID — call the [definitions endpoint](/api/resources/account/methods/definitions) for available options. See [Fields and relationships](/using-the-api/fields-and-relationships/) for value type details.",
			BodyPath: "fields",
		},
		&requestflag.Flag[map[string]any]{
			Name:     "relationships",
